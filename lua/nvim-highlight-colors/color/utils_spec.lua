...
describe('Color Utils', function()
	...
	it('should return color value when receiving oklch', function()
		...
		-- oklch(1 0 0) is very close to white (#FFFFFF or #FEFEFE)
		assert.is_true(hex_value == '#FFFFFF' or hex_value == '#FEFEFE')
	end)

	it('should return color value when receiving oklch with decimals', function()
	...
	it('should return oklch color properties', function()
		local oklch_table = utils.get_oklch_values("oklch(0.5 0.1 240)")
		assert.are.equal(oklch_table[1], 0.5)
		assert.are.equal(oklch_table[2], 0.1)
		assert.are.equal(oklch_table[3], 240)
	end)

	it('should return oklch color properties with percentages', function()
		local oklch_table = utils.get_oklch_values("oklch(50% 0.2 180)")
		assert.are.equal(oklch_table[1], 50)
		assert.are.equal(oklch_table[2], 0.2)
		assert.are.equal(oklch_table[3], 180)
	end)
end)

...
function M.create_window(row, col, bg_color, row_offset)
	...
	pcall(vim.api.nvim_set_hl, 0, highlight_color_name, { bg = colors.get_color_value(bg_color, row_offset) })
...
function M.create_highlight(ns_id, row, start_column, end_column, color, should_colorize_foreground)
	...
	if should_colorize_foreground then
		pcall(vim.api.nvim_set_hl, 0, highlight_group, { fg = color_value })
	else
		local foreground_color = colors.get_foreground_color_from_hex_color(color_value)
		pcall(vim.api.nvim_set_hl, 0, highlight_group, { fg = foreground_color, bg = color_value })
	end
	vim.api.nvim_buf_add_highlight(0, ns_id, highlight_group, row + 1, start_column, end_column)
end
